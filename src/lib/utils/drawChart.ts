...
import { ListingStatMonthlyGroupingItem } from "features/listings/types";

interface DrawChartOptions {
  svgRef: React.RefObject<SVGSVGElement>;
  data: ListingStatMonthlyGroupingItem[];
  title: string;
...
export const drawChart = ({
  ...
  svg
    ...
    .on("mouseover", (event, d) => {
      tooltip
        .style("opacity", 1)
        .html(`<strong>${d.name}</strong><br/>Value: ${formatter(d.amount)}`)
