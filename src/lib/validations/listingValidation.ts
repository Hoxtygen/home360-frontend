import { CreateListingProps } from "typedef";
import { object, string, date, array, Schema } from "yup";

export const initialValues: CreateListingProps = {
  title: "",
  description: "",
  furnishing: "",
  position: "",
...
export const newListingValidationSchema: Schema<CreateListingProps> =
  object().shape({
    title: string().required("Advert title is required"),
    description: string().required("description  is required").max(2000),
    furnishing: string()
      .required("furnishing information is required")
      .max(2000),
    ...
    apartmentInfo: object().shape({
      roomNums: string().required(
        "Number of rooms in the apartment is required"
      ),
